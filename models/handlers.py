...
async def report_error(bot: Bot, user: types.User, error: Exception, step: int = 0):
    ...
    await sync_to_async(BotErrorLog.objects.create)(
        user_id=user.id, error_type=type(error).__name__,
        message=str(error), stack_trace=tb
    )
    msg = f"🚨 <b>BOT ERROR</b>\n👤 {user.full_name}\n📍 Step: {step}\n❌ {str(error)}"
    try:
...
async def cmd_start(message: types.Message, state: FSMContext, bot: Bot):
    try:
        ...
        if not vacs: return await bot.send_message(message.chat.id, "Hozirda bo'sh vakansiyalar yo'q.")
        kb = types.InlineKeyboardMarkup(
            inline_keyboard=[[types.InlineKeyboardButton(text=v.title, callback_data=f"v_{v.id}")] for v in vacs])
        await bot.send_message(message.chat.id, "<b>Carwon HR botiga xush kelibsiz.</b>\n\nVakansiyani tanlang:",
                               reply_markup=kb, parse_mode="HTML")
    except Exception as e:
        await report_error(bot, message.from_user, e, step=0)
...
async def handle_choice(call: types.CallbackQuery, state: FSMContext, bot: Bot):
    ...
    mock_msg = types.Message(message_id=call.message.message_id, date=call.message.date, chat=call.message.chat,
                             from_user=call.from_user, text=val)
...
async def handle_interview(message: types.Message, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        if message.video_note:
            val = message.video_note.file_id
        elif message.voice or message.audio:
            val = (message.voice or message.audio).file_id
        else:
            val = message.text
        answers[q["key"]] = val
        ...
        if next_step < len(QUESTIONS):
            await state.update_data(answers=answers, step=next_step)
            ...
            if next_q.get("type") == "choice":
                ...
                await bot.send_message(chat_id, next_q["text"])
        else:
            ...
            kb = types.InlineKeyboardMarkup(inline_keyboard=[
                [types.InlineKeyboardButton(text="Webda ko'rish 🌐", url=f"{site_url}/candidate/{cand.id}/")]])
            admin_msg = f"🆕 <b>Yangi nomzod!</b>\n\n👤 {cand.full_name}\n📞 {cand.phone}\n💼 {cand.vacancy.title}"
            await bot.send_message(os.getenv("ADMIN_ID"), admin_msg, reply_markup=kb, parse_mode="HTML")
            await state.clear()
    except Exception as e:
        await report_error(bot, message.from_user, e, step)
        await bot.send_message(chat_id, "⚠️ Texnik xatolik yuz berdi. Admin xabardor qilindi.")